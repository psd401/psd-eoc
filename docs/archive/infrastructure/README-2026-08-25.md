...
- Deployment uses only a direct `cdk deploy` command from a locally
  authenticated, short-lived AWS session. The repository has no deployment
  role or deployment automation.
...
- short-lived human AWS credentials targeting account `338414773271` and
  `us-west-2`;
- a current CDK bootstrap in that account;
