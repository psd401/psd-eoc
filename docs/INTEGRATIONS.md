...
| OpenStreetMap raster tiles                 | Optional location-pin map context                             | `configured-unverified` | Public read-only tile configuration only; no credentials or write path exist, no deployment or live availability is verified, and failure leaves canonical location text and posting controls available |
