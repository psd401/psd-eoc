...
| Google Groups                              | Access gating and versioned staff roster snapshots                           | `blocked`               | Source validation targets the retained non-delegated Cloud Identity Groups reader with one readonly scope and immutable email-keyed snapshots, but the App Runner secret reference is not yet approved in deployable CDK and no deployed staff-only end-to-end roster job has run; synthetic tests do not prove provider connectivity                |
