...
import {
  ...
  waitForEventTypePlaywrightPortToClose,
} from './playwright-run';
...
async function main(): Promise<void> {
  ...
  if (
    ...
    throw new Error(
      'The event-type Playwright web server started outside its run workspace.',
    );
  }
  ...
  try {
    ...
    const exitCode = await finalizeEventTypePlaywrightWebServer(
      ...
      waitForEventTypePlaywrightPortToClose,
      () => dropOwnedEventTypePlaywrightDatabase(context),
    );
