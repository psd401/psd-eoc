...
CREATE OR REPLACE FUNCTION public."psd_eoc_insert_authorized_notification_intent"(
  ...
  p_delivery_test_target_set_id uuid,
  p_delivery_test_target_set_version integer,
  p_delivery_test_endpoint_reference_digest varchar(64),
...
BEGIN
  ...
  IF p_purpose = 'activation' THEN
    ...
    WHERE preview."id" = NULLIF(p_authorization ->> 'activationPreviewId', '')::uuid
      ...
      AND preview."delivery_test_target_set_id" IS NOT DISTINCT FROM p_delivery_test_target_set_id
      AND preview."delivery_test_target_set_version" IS NOT DISTINCT FROM p_delivery_test_target_set_version
      AND preview."delivery_test_endpoint_reference_digest" IS NOT DISTINCT FROM p_delivery_test_endpoint_reference_digest
  ...
  INSERT INTO public."notification_intents" (
    ...
    "delivery_test_target_set_version", "delivery_test_endpoint_reference_digest",
    "created_at"
  ) VALUES (
    ...
    p_request_id, p_authorization, p_delivery_test_target_set_id,
    p_delivery_test_target_set_version, p_delivery_test_endpoint_reference_digest,
    p_created_at
  );
...
REVOKE ALL ON FUNCTION
  public."psd_eoc_insert_authorized_notification_intent"(
    ...
    uuid, integer, varchar, timestamp with time zone, timestamp with time zone
  )
