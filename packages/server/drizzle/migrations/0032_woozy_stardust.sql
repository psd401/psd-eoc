CREATE TABLE "sms_provider_io" (
	...
	CONSTRAINT "sms_provider_io_completion_pairing" CHECK (("sms_provider_io"."completion" is null) = ("sms_provider_io"."completed_at" is null)),
	CONSTRAINT "sms_provider_io_completion_object" CHECK ("sms_provider_io"."completion" is null or jsonb_typeof("sms_provider_io"."completion") = 'object')
);
--> statement-breakpoint
CREATE TABLE "sms_retry_schedules" (
	...
	CONSTRAINT "sms_retry_schedules_next_attempt_uq" UNIQUE("next_attempt_id"),
	CONSTRAINT "sms_retry_schedules_attempt_positive" CHECK ("sms_retry_schedules"."next_attempt_number" between 2 and 10),
	CONSTRAINT "sms_retry_schedules_delay" CHECK ("sms_retry_schedules"."delay_milliseconds" between 1 and 3600000),
	CONSTRAINT "sms_retry_schedules_window" CHECK ("sms_retry_schedules"."retry_at" < "sms_retry_schedules"."expires_at")
);
--> statement-breakpoint
ALTER TABLE "sms_provider_io" ADD CONSTRAINT "sms_provider_io_attempt_id_channel_attempts_id_fk" FOREIGN KEY ("attempt_id") REFERENCES "public"."channel_attempts"("id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "sms_retry_schedules" ADD CONSTRAINT "sms_retry_schedules_source_attempt_id_channel_attempts_id_fk" FOREIGN KEY ("source_attempt_id") REFERENCES "public"."channel_attempts"("id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
CREATE INDEX "sms_retry_schedules_retry_at_idx" ON "sms_retry_schedules" USING btree ("retry_at");
--> statement-breakpoint
...
BEGIN
	...
	SELECT
		...
		pg_catalog.count(*) FILTER (WHERE endpoint."channel" = 'sms')::pg_catalog.int4
	INTO endpoint_count, push_count, email_count, sms_count
	...
	IF NOT (
		(endpoint_count BETWEEN 2 AND 12000 AND push_count >= 1 AND email_count >= 1)
		OR (endpoint_count = 1 AND sms_count = 1)
	) THEN
		RAISE EXCEPTION 'Delivery-test target set requires 2-12000 endpoints including push and email, or one SMS canary endpoint'
			USING ERRCODE = '55000';
	END IF;
...
BEGIN
	...
	IF pg_catalog.jsonb_typeof(NEW."channels") IS DISTINCT FROM 'array'
		OR (
			pg_catalog.jsonb_array_length(NEW."channels") NOT BETWEEN 2 AND 3
			AND NOT (
				pg_catalog.jsonb_array_length(NEW."channels") = 1
				AND NEW."channels" -> 0 ->> 'channel' = 'sms'
			)
		)
	THEN
		RAISE EXCEPTION 'Delivery-test report requires push and email channels, or one SMS canary channel'
			USING ERRCODE = '55000';
	END IF;
	...
	IF NOT (
		...
		OR (
			coalesce(pg_catalog.array_length(seen_channels, 1), 0) = 1
			AND 'sms' = ANY(seen_channels)
		)
	) THEN
		RAISE EXCEPTION 'Delivery-test report requires push and email channels, or one SMS canary channel'
			USING ERRCODE = '55000';
...
ALTER TABLE "delivery_test_reports" ADD CONSTRAINT "delivery_test_reports_channels_shape" CHECK (
	...
	and (
		...
		or (
			...
			and jsonb_array_length(jsonb_path_query_array(
				"delivery_test_reports"."channels", '$[*] ? (@.channel == "sms")'
			)) = 1
		)
	)
) NOT VALID;
