...
export interface MediaObjectStoreS3Client {
  getObject(command: GetObjectCommand): Promise<GetObjectCommandOutput>;
  getObjectTagging(
    command: GetObjectTaggingCommand,
  ): Promise<GetObjectTaggingCommandOutput>;
  putObject(command: PutObjectCommand): Promise<PutObjectCommandOutput>;
}
...
async function readBoundedBody(
  ...
  if (body instanceof ReadableStream) {
    return readWebStream(body, maximumBytes);
  }
  if (
    typeof body === 'object' &&
    body !== null &&
    ...
    return readAsyncIterable(body as AsyncIterable<unknown>, maximumBytes);
  }
  throw new MediaObjectStoreError(
    'OBJECT_BODY_UNAVAILABLE',
...
async function assertExistingSanitizedObjectMatches(
  ...
  try {
    output = await client.getObject(command);
  } catch {
  ...
  try {
    const headersMatch =
      output.ContentLength === expected.bytes.byteLength &&
      output.ContentType === expected.contentType &&
      output.CacheControl === SANITIZED_CACHE_CONTROL &&
      metadataMatchesExactly(output.Metadata, expected.metadata);
    if (!headersMatch) {
...
function createAwsDependencies(configuration: MediaObjectStoreConfiguration): {
  ...
  const awsClient = new S3Client({ region: configuration.region });
  const client: MediaObjectStoreS3Client = {
    getObject: (command) => awsClient.send(command),
    getObjectTagging: (command) => awsClient.send(command),
    putObject: (command) => awsClient.send(command),
  };
...
export function createMediaObjectStore(
  ...
  return Object.freeze({
    ...
    async readVerifiedRawObject(
      ...
      if (output.ContentLength !== input.expectedByteLength) {
        await cancelBody(output.Body);
        throw new MediaObjectStoreError(
          'OBJECT_SIZE_MISMATCH',
      ...
      try {
        bytes = await readBoundedBody(output.Body, input.expectedByteLength);
      } catch (error) {
        if (isMediaObjectStoreError(error)) {
          throw error;
        }
      ...
      return Object.freeze({
        ...
        storedContentType: output.ContentType ?? null,
      });
    },
    ...
    async putSanitizedObject(
      ...
      try {
        output = await client.putObject(command);
      } catch (error) {
      ...
      if (output.ChecksumSHA256 !== checksum) {
        throw new MediaObjectStoreError(
          'CHECKSUM_MISMATCH',
          'S3 did not confirm the sanitized media checksum.',
        );
      }
    ...
    async createPrivateReadGrant(
      ...
      try {
        const readUrl = parseHttpsUrl(
          await signer(command, { expiresIn: expiresInSeconds }),
        );
    ...
    async getMalwareScanStatus(storageKey: string): Promise<MalwareScanStatus> {
      ...
      try {
        output = await client.getObjectTagging(command);
      } catch {
        throw new MediaObjectStoreError(
          'STORAGE_UNAVAILABLE',
