    ...
    >(
      await database.execute(sql`
        select
          ...
          has_function_privilege(
            'psd_eoc_app',
            'public.psd_eoc_insert_authorized_notification_intent(uuid, uuid, event_kind, template_mode, notification_purpose, uuid, uuid, roster_population, uuid, integer, jsonb, invocation_source, uuid, jsonb, timestamp with time zone, timestamp with time zone)',
            'EXECUTE'
          ) as "functionExecute"
      `),
    );
