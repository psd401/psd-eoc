...
The baseline cannot send a notification. It creates durable queues but no
queue consumers, provider-send permissions, subscriptions, schedules, or live
...
- Retained, non-expiring CloudWatch log groups and encrypted operations and
  critical-alarm topics. Alarms and subscriptions belong to phase 5.
...
- separate approval before creating the DKIM DNS records or changing any live
...
After the stack completes, copy `SesDkimRecordName1..3` and
`SesDkimRecordValue1..3` into the manually reviewed DNS change for external
task E2. Complete the separately approved database bootstrap before enabling
database-backed use; until then, those requests fail closed. Do not infer SES
production access or delivery verification from stack completion. No
