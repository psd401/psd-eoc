...
async function runScenario(options: ScenarioOptions): Promise<ScenarioResult> {
  ...
  const outerIdentity = {
    ...
    Arn: 'arn:aws:sts::338414773271:assumed-role/psd-eoc-exploration-smoke-github-deploy/psd-eoc-domain-123',
  ...
  it('fails closed when the association response names a different service', async () => {
    const result = await runScenario({
      associateResponse: {
        ...
        DNSTarget: dnsTarget,
        ServiceArn:
          'arn:aws:apprunner:us-west-2:338414773271:service/not-the-service/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
      },
      customDomainResponses: [response([])],
      mode: 'associate-if-absent',
    });
